// Global functionality for Din gulvmand v/Nikolaj Nyhuus
(function(){
  const doc=document;
  ...
  const GA_ID='GA_MEASUREMENT_ID'; // Replace with actual GA4 ID
  ...
  const showBanner=()=>{
    ...
    if(cookieDetails) cookieDetails.setAttribute('hidden','');
  };
  ...
  const forms=[
    {id:'quote',fields:{name:{required:true},email:{required:true,email:true},phone:{},type:{},area:{number:true},city:{},message:{required:true}},subject:'Tilbudsanmodning',formatter:data=>`Navn: ${data.name||''}%0DE-mail: ${data.email||''}%0DTelefon: ${data.phone||''}%0DOpgavetype: ${data.type||''}%0DAreal: ${data.area||''}%0DBy: ${data.city||''}%0DBesked:%0D${data.message||''}`},
    {id:'contact-form',fields:{name:{required:true},email:{required:true,email:true},message:{required:true}},subject:'Kontaktbesked',formatter:data=>`Navn: ${data.name||''}%0DE-mail: ${data.email||''}%0DBesked:%0D${data.message||''}`}
  ];
